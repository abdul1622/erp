...
if(id == null &&model_name=='product' && 'multiple_parts' in data == false){
   ...
   .then(response =>{ 
       ...
       if(data.status == 'success'){
        ...
        document.getElementById('dm-filter').style.display = 'block'
           model_form_get()
        //    document.querySelector('.messages').innerHTML = `<p class='text-success'>${model_name} ${result} successfully`
        }
        else{
            console.log(data)
            error = ''
   ...
   function party_type_change(element){
    ...
    fetch( `${host}/api/dropdown/?model=product&&name=product_name&&filter_by=product_type&&filter_value=semi-finished`,{
        method: 'GET',
        headers: {
            'Accept': 'application/json',
            'Content-Type': 'application/json',
